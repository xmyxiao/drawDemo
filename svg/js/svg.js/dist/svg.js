...
, toLine: function() {
    return {
      x1: Number(this.value[0][0]).toFixed(2)
    , y1: Number(this.value[0][1]).toFixed(2)
    , x2: Number(this.value[1][0]).toFixed(2)
    , y2: Number(this.value[1][1]).toFixed(2)
    }
  }
  // Get morphed array at given position
, at: function(pos) {
...
, extend: {
    // Move over x-axis
    x: function(x) {
      return this.attr('x', Number(x).toFixed(2))
    }
    // Move over y-axis
  , y: function(y) {
      return this.attr('y', Number(y).toFixed(2))
    }
...
, extend: {
  ...
  , size: function(width, height) {
      var p = proportionalSize(this, width, height)
			if(this.type == "line"){
				var lineArr = this.array().size(p.width, p.height).toLine();
				if(height == "0"){
					lineArr.y1 = lineArr.y2 = $(this.node).attr("y1");
				}else if(width == "0"){
					lineArr.x1 = lineArr.x2 = $(this.node).attr("x1");
				}
				return this.attr(lineArr);
			}else{
...
, extend: {
  ...
  , y: function(y) {
      ...
      return this.attr('y', Number(typeof y === 'number' ? y + o : y).toFixed(2))
    }
